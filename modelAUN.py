...
class AdversPatchWorkModel(Model):
  def __init__(self,cropper,
    ...
    self.mode = 'applySegmenter'
 # self.mode = 'trainSegmenter_supervised'
 # self.mode = 'trainSegmenter_unsupervised'
 # self.mode = 'trainDiscriminator'
  ...
  def call(self, inputs, training=False, 
           lazyEval=None, stitch_immediate=False, testIT=False):
      
      x = inputs
      if self.mode ==  'applySegmenter' or self.mode ==  'trainSegmenter_supervised':
          x = self.callSegmenter(x,training=training,lazyEval=lazyEval, stitch_immediate=stitch_immediate, testIT=testIT)
      elif self.mode ==  'trainSegmenter_unsupervised':
          x = self.callSegmenter(x,training=training,lazyEval=lazyEval, stitch_immediate=stitch_immediate, testIT=testIT)
          x = self.discriminator(x[-1],training=False)
      elif self.mode ==  'trainDiscriminator':
           x = self.discriminator(x['input0'],training=training)
      else: 
          x = None
  ...
  def callSegmenter(self, inputs, training=False, lazyEval=None, stitch_immediate=False, testIT=False):
...
#%%        
    ...
    for i in range(num_its):
        ...
        with tf.device(DEVCPU):            
            ...
            if labelset is not None:
               ...
               targetdata = tf.ones([inputdata['input0'].shape[0],1])
  ...
  def trainDiscriminator(self,
    ...
    def getSample(theset,subset,valid=False):
        tset = [theset[i] for i in subset]
        rset = None
        ...
        with tf.device(DEVCPU):    
            ...
            if traintype == 'random':
                ...
                if valid:
                    np = valid_num_patches
                c = self.cropper.sample(tset,None,resolutions=rset,generate_type='random', 
                                        num_patches=np,augment=augment,balance=balance,dphi=dphi)
            elif traintype == 'tree':
                c = self.cropper.sample(tset,None,resolutions=rset,generate_type='tree_full', jitter=jitter,
                                        jitter_border_fix=jitter_border_fix,augment=augment,balance=balance,dphi=dphi)
        return c
    ...
    if loss is not None:
        if optimizer is None:
            optimizer = tf.optimizers.Adam(learning_rate=0.0001, beta_1=0.9, beta_2=0.999, amsgrad=True)
        print("compiling ...")
        self.compile(loss=loss, optimizer=optimizer)

    trainidx = list(range(len(trainset)))
    testidx = list(range(len(testset)))

                     
            
    for i in range(num_its):
        print("----------------------- discrim ------------------ iteration:" + str(i))
        
        ### sampling
        print("sampling patches for training")
        start = timer()
           
        ctrain = getSample(trainset,trainidx)
        ctest = getSample(testset,testidx)
        ...
        with tf.device(DEVCPU):            
            traindata = ctrain.getInputData(sampletyp)
            testdata = ctest.getInputData(sampletyp)
        targetdata = tf.concat([tf.ones([traindata['input0'].shape[0],1]),tf.zeros([testdata['input0'].shape[0],1])],0)
        data = {}
        for k in traindata:
            data[k] = tf.concat([traindata[k],testdata[k]],0)
        print("starting training")
        start = timer()
        self.fit(data,targetdata,
                  epochs=epochs,
                  verbose=verbose,
                  steps_per_epoch=steps_per_epoch,
                  batch_size=batch_size,
